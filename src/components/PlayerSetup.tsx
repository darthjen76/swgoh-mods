...
export default function PlayerSetup() {
  ...
  const query = usePlayer(submitted ? code : '')
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: '#0a0e1a' }}>
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <p className="text-slate-500 text-xs mt-1">
              9 chiffres — visible dans ton profil in-game
            </p>
          </div>

          <button
            type="submit"
            disabled={query.isFetching || code.length < 9}
          ...
          >
            {query.isFetching ? (
              <span className="flex items-center justify-center gap-2">
                ...
                Chargement… (peut prendre 30s au démarrage)
              </span>
            ) : 'Charger mon roster'}
          </button>

          {query.isError && (
            <div className="p-3 rounded-lg text-red-400 text-sm space-y-1"
        ...
        <p className="text-center text-slate-600 text-xs mt-8">
          Données chargées via l'API officielle SWGOH
        </p>
      </div>
    </div>
  )
