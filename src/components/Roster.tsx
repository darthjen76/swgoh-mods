...
function CharacterCard({ char, onClick }: { char: Character; onClick: () => void }) {
  const speedTotal = char.mods.reduce((sum, mod) => {
    return sum + (mod.secondary_stats.find((s) => s.stat_id === 5)?.value ?? 0)
  }, 0)
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1">
            <span className="text-xs text-blue-400 font-semibold">
              {speedTotal > 0 ? `+${Math.round(speedTotal)} SPD` : 'Pas de mods'}
            </span>
            <span className="text-xs text-slate-600">{char.mods.length}/6 mods</span>
          </div>
        </div>
    ...
    <div className="p-4 max-w-2xl mx-auto">
      <div className="mb-4">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-xs text-slate-500">{playerData.characters.length} personnages</span>
        </div>
        <input
          type="text"
